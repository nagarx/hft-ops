...
from hft_ops.manifest.loader import load_manifest  # noqa: F401  (kept for back-compat — test fixtures may import via this path)
from hft_ops.paths import PipelinePaths


# Phase R-16c Sub-cycle 4b SSoT extraction (2026-05-12): the implementation
# moved to `hft_ops.ledger.signal_dir.resolve_signal_dir` to support consumption
# by `r16c_analysis.py` (2nd consumer triggers §0 reuse-first extraction).
# This alias preserves the dotted import path
# `hft_ops.ledger.statistical_compare._resolve_signal_dir` for the 22+ existing
# test references (12 mock-patch sites in `test_statistical_compare.py` + 10
# direct call sites in `test_resolve_signal_dir.py`). Both paths point at the
# same underlying function — zero behavior change, zero test churn.
_resolve_signal_dir = resolve_signal_dir
...
# `_resolve_signal_dir` body moved to `hft_ops.ledger.signal_dir.resolve_signal_dir`
# (Phase R-16c Sub-cycle 4b SSoT extraction, 2026-05-12). The dotted-import-path
# alias `hft_ops.ledger.statistical_compare._resolve_signal_dir` is preserved at
# the top of this module (assignment from imported `resolve_signal_dir`) so the
# 22+ existing test references continue working unchanged.
