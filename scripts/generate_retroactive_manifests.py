...
def classify_experiment(output_dir: Path) -> Tuple[str, Optional[Path]]:
    ...
    for candidate in (
        "e4_baselines_H60.json",
        "e5_baselines_H10.json",
...
def generate_manifest(
    ...
    if trainer_cfg_path.exists():
        trainer_cfg_rel = str(trainer_cfg_path.relative_to(pipeline_root))
    ...
    manifest: Dict = {
        ...
        "stages": {
            ...
            # 2026-08-15: `validation` must be declared even though it is disabled.
            # The §13 gate parameters became REQUIRED-when-enabled, and the loader
            # raises MissingGatePolicyError for an ENABLED validation stage with no
            # declared policy. A generator that omits the key entirely produced
            # manifests its own loader rejected — and, worse, a re-run would have
            # silently regenerated away the 34 hand-declared policies.
...
def run_backfill(
    ...
    cmd = [
        ...
        "completed",
        "--notes",
        "Generated by generate_retroactive_manifests.py",
    ]
    if metrics_file:
...
def main() -> None:
    ...
    trainer_configs_dir = (
        pipeline_root / "lob-model-trainer" / "configs" / "experiments"
    )
    ...
    candidates = sorted(
        d for d in outputs_dir.iterdir() if d.is_dir() and not d.name.startswith("_")
    )
    if args.include:
    ...
    for output_dir in candidates:
        ...
        if args.backfill:
            ok, out = run_backfill(
                manifest_path, record_type, metrics_file, hft_ops_dir
            )
