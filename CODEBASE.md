...
> **Version**: 0.3.0 (`pyproject.toml:7` ⟺ `src/hft_ops/__init__.py:14` — AGREE. The former `0.3.0-dev` pre-release suffix (VERSIONING.md R5) and the `pyproject 0.3.0-dev` vs `__version__ 0.2.0` split (the worked example behind VERSIONING.md R9) were BOTH closed in commit `3655f68`; do not re-report them as open.) | **Schema**: 3.0 (Phase G G.6.A bump 2.2 → 3.0 MAJOR per CLAUDE.md root rule: any modification to stable features 0-97 = BREAKING) | **Tests**: run `.venv/bin/python -m pytest --collect-only -q` for the live count — hand-typed counts are banned here per hft-rules §11 (the prior banner count contradicted §6 for weeks). | **Last comprehensive codebase truth-up**: 2026-07-07 (Curation Phase-2 TRUTH — test-count/dependency/data-flow drift fixes + monitor denylist hardening)
>
> **Scoped documentation update (2026-08-02):** added the Databento raw-input cache-custody limitation below; no source, cache behavior, or test surface changed.
>
...
>
> **Databento raw-input custody limitation (2026-08-02):**
> `raw_input_manifest_hash` hashes only the adjacent local `manifest.json`
> bytes. The current ingest session manifest can contain size-only skips and
> checksums only for objects newly downloaded in that session; it is not a
> complete current-directory checksum ledger. Therefore this cache input is
> content-addressed over the *manifest file*, not over every selected raw DBN,
> and it does not by itself pin raw-data lineage or present-byte identity. The
> source comment that says raw lineage is pinned “by CONTENT” must be read with
> this narrower meaning. Until a cache key binds the exact release-scoped raw
> object set and per-file hashes, require an external catalog/release pin and
> independently verified input hashes for a custody-sensitive reproduction.
>
...
The bridge between hft-ops and the current research arc: each discovery probe in the **seven scanned trees** (`glbx_discovery/`, `xsec_equity_discovery/`, `nvda_discovery/`, `opra_discovery/`, `pead_discovery/`, `crypto_discovery/`, `multiday_discovery/` — the `DISCOVERY_TREES` constant) emits a normalized verdict JSON, and this subsystem makes those verdicts queryable, fingerprinted, and drift-monitored alongside training runs — one experiment × verdict × provenance × drift surface. **Scope notes**: (1) a `SKIP_PATH_PARTS` guard drops any results-glob hit under a `.venv/` or `site-packages/` path component before parsing — `crypto_discovery/.venv` embeds third-party test fixtures (statsmodels) matching `**/results/*.json` that would otherwise render as phantom UNRESOLVED rows via the CommonCoreAdapter catch-all; this guard is what licensed adding the crypto/multiday trees (curation Phase 3, 2026-07-07); (2) `pead_discovery/` is design-only today (no `results/` ever emitted) — kept in scope as a zero-cost no-op so a future run needs no code change.
