...
> **Version**: 0.3.0-dev | **Schema**: 2.2 | **Tests**: 498 pass + 5 pre-existing env-dependent failures (`test_validation_stage.py` — hft-feature-evaluator not installed in fresh env; unrelated to Phase 8C-α) | **Last Updated**: 2026-04-20 (Phase 8C-α Stage C.3 + 2-round post-audit — `persist_post_stage_artifacts` content-addressed ledger routing + `_POST_STAGE_ARTIFACT_PATTERNS` registry + atomic tmp+rename copy + same-record SHA dedup + regression test locking `artifacts[]` EXCLUDED from `compute_fingerprint`)
>
