...
> **Version**: 0.3.0-dev | **Schema**: 2.2 | **Tests**: 500 pass + 5 pre-existing env-dependent failures (`test_validation_stage.py` — hft-feature-evaluator not installed in fresh env; unrelated to Phase 8C-α) | **Last Updated**: 2026-04-20 (Phase 8C-α Integration Close-Out + round-3 post-audit hot-fixes — Q2 content-hash unification: router now uses `validated.content_hash()` canonical-JSON SHA; Q6 API-insulated `_assert_no_field_in_canonical_input` test helper; `importance` + `artifacts` added to `_extract_fingerprint_fields` blacklist; **double-strip bug fix**: overrides were injecting observation-tier keys AFTER the initial blacklist strip — caught by round-3 non-vacuous regression tests; `_extract_fingerprint_fields` now runs pass-2 after overrides merge; preserves Stage C.3 + round-1 post-audit: atomic tmp+rename copy + same-record SHA dedup + defensive try/except + stage_name propagation)
>
